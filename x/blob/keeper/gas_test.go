...
func TestChangingGasParam(t *testing.T) {
	msg := types.MsgPayForBlobs{BlobSizes: []uint32{1024}}
	k, stateStore, _ := CreateKeeper(t, appconsts.LatestVersion)
	tempCtx := sdk.NewContext(stateStore, false, log.NewNopLogger())

	ctx1 := sdk.NewContext(stateStore, false, log.NewNopLogger())
	...
	ctx2 := sdk.NewContext(stateStore, false, log.NewNopLogger())
