...
func TestCheckTx(t *testing.T) {
	...
	tests := []test{
		{
			name:      "normal transaction, CHECK_TX_TYPE_CHECK",
			checkType: abci.CHECK_TX_TYPE_CHECK,
			getTx: func() []byte {
		...
		},
		{
			...
			checkType: abci.CHECK_TX_TYPE_RECHECK,
			getTx: func() []byte {
		...
		{
			name:      "PFB with no blob, CHECK_TX_TYPE_CHECK",
			checkType: abci.CHECK_TX_TYPE_CHECK,
			getTx: func() []byte {
		...
		{
			name:      "normal blobTx w/ multiple blobs, CHECK_TX_TYPE_CHECK",
			checkType: abci.CHECK_TX_TYPE_CHECK,
			getTx: func() []byte {
				signer := createSigner(t, kr, accs[4], encCfg.TxConfig, 5)
				_, blobs := blobfactory.RandMsgPayForBlobsWithSigner(unsafe.NewRand(), signer.Account(accs[4]).Address().String(), 10_000, 10)
		...
		},
		{
			...
			getTx: func() []byte {
				...
				_, blobs := blobfactory.RandMsgPayForBlobsWithSigner(unsafe.NewRand(), signer.Account(accs[5]).Address().String(), 1_000, 1)
		...
		},
		{
			...
			getTx: func() []byte {
				...
				_, blobs := blobfactory.RandMsgPayForBlobsWithSigner(unsafe.NewRand(), signer.Account(accs[6]).Address().String(), 10_000, 1)
		...
		},
		{
			...
			getTx: func() []byte {
				...
				_, blobs := blobfactory.RandMsgPayForBlobsWithSigner(unsafe.NewRand(), signer.Account(accs[7]).Address().String(), 100_000, 1)
		...
		},
		{
			...
			getTx: func() []byte {
				...
				_, blobs := blobfactory.RandMsgPayForBlobsWithSigner(unsafe.NewRand(), signer.Account(accs[8]).Address().String(), 1_000_000, 1)
		...
		},
		{
			...
			getTx: func() []byte {
				...
				_, blobs := blobfactory.RandMsgPayForBlobsWithSigner(unsafe.NewRand(), signer.Account(accs[9]).Address().String(), 2_000_000, 1)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp, err := testApp.CheckTx(&abci.CheckTxRequest{Type: tt.checkType, Tx: tt.getTx()})
			require.NoError(t, err)
			require.Equal(t, tt.expectedABCICode, resp.Code, resp.Log)
		})
	}
}
