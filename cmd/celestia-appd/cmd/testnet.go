...
Example:
	...
	cmd.Flags().String(flagStakingDenom, appconsts.BondDenom, "Default staking token denominator")
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		valStr, err := valAddrCodec.BytesToString(addr)
		if err != nil {
			return err
		}
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		var x genutiltypes.AppGenesis
		if err := json.Unmarshal(bz, &x); err != nil {
			return err
		}
		// set app version
		x.Consensus.Params.Version = cmttypes.VersionParams{App: appconsts.LatestVersion}
		if err := x.SaveAs(genFile); err != nil {
			return err
		}
	}
...
func NewTestNetworkFixture() network.TestFixture {
	...
	defer os.RemoveAll(dir) // nolint: test code

	cApp := app.New(log.NewNopLogger(), dbm.NewMemDB(), nil, time.Second, simtestutil.NewAppOptionsWithFlagHome(dir))

	appCtr := func(val network.ValidatorI) servertypes.Application {
		return app.New(
			val.GetCtx().Logger, dbm.NewMemDB(), nil, time.Second, simtestutil.NewAppOptionsWithFlagHome(val.GetCtx().Config.RootDir),
	...
	return network.TestFixture{
		...
		GenesisState:   cApp.DefaultGenesis(),
		EncodingConfig: moduletestutil.TestEncodingConfig{
			InterfaceRegistry: cApp.GetEncodingConfig().InterfaceRegistry,
			Codec:             cApp.AppCodec(),
			TxConfig:          cApp.GetEncodingConfig().TxConfig,
			Amino:             cApp.GetEncodingConfig().Amino,
		},
	}
}
