	...
	testCases := []testCase{
		{
			...
			wantGasConsumed: uint64(1 * share.ShareSize * appv4.GasPerBlobByte), // 1 share * 512 bytes per share * 8 gas per byte= 4096 gas
		},
		{
			name:            "100 byte blob", // occupies 1 share
			msg:             types.MsgPayForBlobs{BlobSizes: []uint32{100}},
			wantGasConsumed: uint64(1 * share.ShareSize * appv4.GasPerBlobByte),
		},
		{
			...
			wantGasConsumed: uint64(3 * share.ShareSize * appv4.GasPerBlobByte), // 3 shares * 512 bytes per share * 8 gas per byte = 12288 gas
		},
		{
		...
		},
		{
			...
			wantGasConsumed: uint64(6 * share.ShareSize * appv4.GasPerBlobByte), // 6 shares * 512 bytes per share * 8 gas per byte = 24576 gas
		},
	}
